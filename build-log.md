...
Rationale: Saw someone on GitHub one day who had made a fake company with a couple of markdown files and screenshotted for later Git projects. The idea stuck because a fictional company removes the constraint of writing about a real public incident. Since the company is mine to define the postmortems, policies, risk registers, etc. can be whatever I want them to be.
...
Consequences: Project scope more than doubles. I'm now responsible for design decisions, learning Astro, GitHub Pages deployment and DNS configuration again. Not my first time with GitHub Pages or DNS (I have a portfolio site and two 
domains) but far from a mastermind. The resulting portfolio piece is more dynamic (writing + design + technical) instead of just a document.
...
Consequences: I can speak at length about the fictional company's product, customers, regulations, etc. Slight risk that reviewers read this as a niche/pigeonhole instead of a deliberate choice. This needs to be framed clearly in the project essay.

---

**Picked the name Fenn (after Furrow and Fellow).**

Context: Needed a name for the company. Wanted something in the realm of land, countryside, plants, green.

Decision: Started with Furrow. Set up the GitHub Pages repo, wrote the README, configured the basics. Checked UK trademark and Companies House but missed that Furrow wasn't usable. Switched to Fellow and rebuilt everything. Same issue. Checked, but missed something. Landed on Fenn and rebuilt a third time.

Rationale: Fenn Agritech passed all the availability checks the other two failed. It also was short which I wanted and matched the theme I was going for which was more earthy than techy.

Consequences: Lost a whole day doing setup and company research twice. Learned the hard way that running checks isn't enough if you don't know how to read them: class designations, what counts as a real conflict vs. unrelated, what shows up in Companies House vs. trademark databases. Final name (Fenn) is good. Learned what a fenn even is and the part they play in UK agriculture. Going forward: check trademark AND domain availability properly before any setup work on a project that uses a name.
